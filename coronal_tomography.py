...
class CoronalTomography(object):
    ...
    def observations_and_reference_system(self, directory=None, reduction=1, n_pixels_integration=64):
        ...
        if (self.hyperparameters['obs'] == 'predsci_time'):
            self.dataset = dataset.DatasetPREDSCI_Time(n_angles=20, device=self.device, FOV=6.3)
        ...
        self.max_angle = torch.max(self.dataset.angles)
        self.hyperparameters['max_angle'] = self.max_angle
        ...
        self.weight = torch.exp(self.p_POS - 5.0) / self.p_POS
        self.weight = self.weight.reshape((self.dataset.n_pixels, self.dataset.n_pixels))        
    ...
    def init_optimize(self, optimizer=True):
        ...
        if (optimizer):
            ...
            # Scheduler
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, self.n_epochs, eta_min=0.3*lr)
    ...
    def synth_old(self, XYZ_LOS, M_rot, angles, omegapB, mask, compute_electron_LOS=False):
        ...
        if (self.time):
            ang = angles[:, None, None].repeat(1, n_points, 1)
            inp = torch.cat([ang / 200.0, XYZ_rotated / self.r_max], dim=-1)
        else:
    ...
    def synth(self, XYZ_LOS, M_rot, angles, omegapB, mask, compute_electron_LOS=False):
        ...
        if (self.time):
            ...
            n_points = XYZ_rotated.shape[1]
            ang = angles[:, None, None].repeat(1, n_points, 1)            
            inp = torch.cat([1.8 * ang / 200.0 - 0.9, XYZ_rotated / self.r_max], dim=-1)
        else:
            inp = XYZ_rotated / self.r_max
    ...
    def train_epoch(self, epoch, optimizer=True):
        ...
        for batch_idx, (obspB, M_rot, angles) in enumerate(t):
            ...
            # ax[0].imshow(np.sqrt(obspB[0,:,:]))
            # ax[1].pcolormesh(self.XYZ_LOS[0,:,:,1], self.XYZ_LOS[0,:,:,2], np.sqrt(np.flipud(obspB[0,:,:]).T))
            # breakpoint()
            ...
            impB = torch.zeros_like(obspB_patch)
            ...
            for i in range(ni):
                for j in range(nj):
                    ...
                    if (mask.sum() != 0):
                        ...
                        impB[:, i, j, :, :] = impB_patch.detach().cpu()                    
            ...
            impB_full = impB.permute(0, 3, 4, 1, 2).reshape((-1, self.patch_size*self.patch_size, ni*nj))
            impB_full = F.fold(impB_full, (self.dataset.n_pixels, self.dataset.n_pixels), self.patch_size, stride=self.patch_size).squeeze(1)

            self.impB.append(impB_full)
            
            min_Ne = min(torch.min(Ne).item(), min_Ne)
            max_Ne = max(torch.max(Ne).item(), max_Ne)
            if (optimizer):
                ...
                if (NVIDIA_SMI):
                    ...
                    t.set_postfix(epoch=f'{epoch}/{self.n_epochs}', lr=f'{current_lr:8.5f}', loss_avg=f'{loss_avg:7.5f}', maxne=f'{max_Ne:5.1e}', minne=f'{min_Ne:5.1e}', gpu=usage.gpu, memfree=f'{memory.free/1024**2:5.1f} MB', memused=f'{memory.used/1024**2:5.1f} MB')
                else:
    ...
    def optimize(self):
        ...
        # (X, Y, Z, 3) -> (X, Ny, Nz, 3, Py, Pz)
        # Ny, Nz -> size of the patches
        # Py, Pz -> number of patches
        self.XYZ_LOS_patch = self.XYZ_LOS.unfold(2, self.patch_size, self.patch_size)                                               # (X, Y, Pz, 3, Nz)
        self.XYZ_LOS_patch = self.XYZ_LOS_patch.unfold(1, self.patch_size, self.patch_size)                                         # (X, Ny, Nz, 3, Pz, Py)
        self.XYZ_LOS_patch = self.XYZ_LOS_patch.permute(0, 5, 4, 3, 1, 2)                                                           # (X, Ny, Nz, 3, Py, Pz)

        # omega : (X, Y, Z) -> (X, Ny, Nz, Py, Pz)
        # mask  : (Y, Z) -> (Ny, Nz, Py, Pz)        
        self.omegapB_patch = self.omegapB.unfold(2, self.patch_size, self.patch_size).unfold(1, self.patch_size, self.patch_size).permute(0, 1, 2, 4, 3)        # (X, Py, Pz, Ny, Nz)
        self.mask_patch = (~self.mask).unfold(1, self.patch_size, self.patch_size).unfold(0, self.patch_size, self.patch_size).permute(0, 1, 3, 2).float()      # (Py, Pz, Ny, Nz) 
        self.weight_patch = self.weight.unfold(1, self.patch_size, self.patch_size).unfold(0, self.patch_size, self.patch_size).permute(0, 1, 3, 2)             # (Py, Pz, Ny, Nz)
    ...
    def synthesize(self):
        ...
        self.XYZ_LOS_patch = self.XYZ_LOS_patch.permute(0, 5, 4, 3, 1, 2)        
...
if (__name__ == '__main__'):
    ...
    corona = CoronalTomography(gpu=0, checkpoint='models/lasco_siren_h256_nh8_w15.0.pth')

    corona.observations_and_reference_system('datapB', reduction=8, n_pixels_integration=64)
