...
class CoronaTomography(object):
    ...
    def optimize(self, model_hyperparameters, batch_size, n_epochs, patch_size=None, lr=6e-3):
            ...
            for batch_idx, (obspB, M_rot) in enumerate(t):
                ...
                for i in range(ni):
                    ...
                    for j in range(nj):
                        ...
                        imB, impB_patch, Ne = self.synth(XYZ_LOS, M_rot, omegapB)
                                                                        
                        # loss in logarithm
                        # loss = torch.mean( weight[None, :, :] * mask[None, :, :] * (torch.log(obspB + 1e-10) - torch.log(impB_tmp + 1e-10))**2)

                        # Loss in linear
                        
                        loss = torch.mean( weight[None, :, :] * mask[None, :, :] * (obspB - impB_patch)**2)
                        
                        # loss = torch.mean( weight[None, :, :] * mask[None, :, :] * (obspB - impB_tmp)**2)
                        # loss = torch.sum( mask[None, :, :] * (obspB - impB_tmp)**2)
                        # loss = torch.mean( mask[None, :, :] * torch.abs(torch.log(obspB + 1e-10) - torch.log(impB_tmp + 1e-10)))

                        total_loss += loss.item() / (ni * nj * len(t))
                        
                        loss.backward()

                        impBj[j] = impB_patch[..., None]
                ...
                if (NVIDIA_SMI):
                    ...
                    t.set_postfix(epoch=f'{epoch}/{n_epochs}', lr=f'{current_lr:8.5f}', loss_b=f'{current_loss:5.3f}', loss_t=f'{total_loss:5.3f}', maxne=f'{max_Ne:5.1e}', minne=f'{min_Ne:5.1e}', gpu=usage.gpu, memfree=f'{memory.free/1024**2:5.1f} MB', memused=f'{memory.used/1024**2:5.1f} MB')
                else:
...
if __name__ == '__main__':
    ...
    hyperparameters = {
        ...
        'logNe_range' :[-5.0, 18.0],
    }

    imB, impB = corona.optimize(hyperparameters, batch_size=4, n_epochs=50, patch_size=32, lr=2e-4)
