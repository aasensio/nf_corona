...
def do_plot_radial(corona, model, r, n_pixels, ax, cmap, norm, label=''):
    ...
    for i in range(len(r)):
        ...
        im = ax[i].pcolormesh(Phi * 180.0 / np.pi, Theta * 180.0 / np.pi, np.log10(Ne_cap[i]), cmap=cmap[i], norm=norm[i])
        # if (label != ''):
...
def do_plot_vertical_x(corona, model, x_cap, n_pixels, ax, cmap, norm, label=''):
    ...
    for i in range(len(x_cap)):
        ...
        im = ax[i].pcolormesh(Y, Z, (~corona.mask) * np.log10(Ne_cap[i]), cmap=cmap[i], norm=norm[i])
...
def do_plot_vertical_y(corona, model, y_cap, n_pixels, ax, cmap, norm, label=''):
    ...
    for i in range(len(y_cap)):
        # im = ax[i].imshow((~corona.mask) * np.log10(Ne_cap[i]), extent=[-7, 7, -7, 7], aspect='equal', cmap=cmap[i], norm=norm[i])
        im = ax[i].pcolormesh(X, Z, (~corona.mask) * np.log10(Ne_cap[i]), cmap=cmap[i], norm=norm[i])
        circ = Circle((0, 0), 1.0, color='yellow')
        ax[i].add_patch(circ)
        if (label != ''):
...
def do_plot_longitude_polar(corona, model, longitude, n_pixels, ax, cmap, norm, label=''):
    ...
    for i in range(len(longitude)):
        ax[i].pcolormesh(theta, r, np.log10(Ne_cap[i]), edgecolors='face')
...
def do_plot_latitude(corona, model, latitude, n_pixels, ax, cmap, norm, label=''):
    ...
    Ne_cap = latitudinal_caps(corona, model, latitude, n_pixels)
    
    for i in range(len(latitude)):
        im = ax[i].imshow(np.log10(Ne_cap[i]), extent=[0.0, 360.0, 2.3, 5.8], aspect='auto', cmap=cmap[i], norm=norm[i])        
        if (label != ''):
            ax[i].text(25, 60, f'{label}', color='white', weight='bold', size='large')
...
def doplot_radial_all():
    ...
    for n_hidden in n_hidden_list:
        for dim_hidden in dim_hidden_list:
            ...
            do_plot_radial(corona, corona.model, r, n_pixels=64, ax=ax[1, :], cmap=cmap, norm=norm, label='w$_0$=5')

            corona.load_checkpoint(checkpoint=f'models/predsci_siren_h{dim_hidden}_nh{n_hidden}_w15.0.pth')
            corona.init_optimize(optimizer=False)
            corona.load_weights()
            do_plot_radial(corona, corona.model, r, n_pixels=64, ax=ax[2, :], cmap=cmap, norm=norm, label='w$_0$=15')

            corona.load_checkpoint(checkpoint=f'models/predsci_siren_h{dim_hidden}_nh{n_hidden}_w30.0.pth')
            corona.init_optimize(optimizer=False)
            corona.load_weights()
            do_plot_radial(corona, corona.model, r, n_pixels=64, ax=ax[3, :], cmap=cmap, norm=norm, label='w$_0$=30')

            for i in range(3):
                ax[0, i].text(25, 60, f'r={r[i]:4.2f}', color='white', weight='bold', size='large')
                if (i == 0):
                    ax[0, i].text(25, 40, f'(n,h)={n_hidden,dim_hidden}', color='white', weight='bold', size='large')
...
def doplot_angular():
    ...
    do_plot_latitude(corona, corona.dataset.model, lat, n_pixels=64, ax=ax1[0, :], cmap=cmap, norm=norm)    
    do_plot_latitude(corona, corona.model, lat, n_pixels=64, ax=ax1[1, :], cmap=cmap, norm=norm)

    for i in range(3):
        ax1[0, i].text(20, 5, f'lat={long[i]:4.2f}', color='white', weight='bold', size='large')        
    ...
    do_plot_longitude(corona, corona.dataset.model, long, n_pixels=64, ax=ax2[0, :], cmap=cmap, norm=norm)    
    do_plot_longitude(corona, corona.model, long, n_pixels=64, ax=ax2[1, :], cmap=cmap, norm=norm)
...
def doplot_lasco(checkpoint):
    
    corona = coronal_tomography.CoronalTomography(gpu=1, checkpoint=checkpoint)

    corona.observations_and_reference_system(directory='data', reduction=8, n_pixels_integration=64)
    ...
    fig, ax = pl.subplots(nrows=3, ncols=1, sharex=True, sharey=True, constrained_layout=True)
    
    do_plot_radial(corona, corona.model, r, n_pixels=64, ax=ax, cmap=cmap, norm=norm, label='w$_0$=15')
    ...
    # fig, ax = pl.subplots(nrows=1, ncols=2, constrained_layout=True)
    
    # phi = torch.tensor([0.0 * np.pi / 180.0])
    # do_plot_polar(corona, corona.model, r, phi, n_pixels=64, ax=ax[0], cmap=cmap, norm=norm, label='x')

    # phi = torch.tensor([90.0 * np.pi / 180.0])
    # do_plot_polar(corona, corona.model, r, phi, n_pixels=64, ax=ax[1], cmap=cmap, norm=norm, label='x')
...
def doplot_lasco_synthesis(checkpoint):
    ...
    obspB = np.log10(mask * corona.dataset.obspB[::4, :, :] + 1e-13)
    impB = np.log10(mask * impB_all[::4, :, :] * 1e10 + 1e-13)
    angles = corona.dataset.angles[::4]
    loop = 0
    fov = corona.dataset.FOV

    fig, ax = pl.subplots(figsize=(12,14), nrows=6, ncols=6, sharex=True, sharey=True, constrained_layout=True)
    
    cmap = mpl.cm.get_cmap('viridis')
    norm = mpl.colors.Normalize(-1.0, 1.0)
    
    for i in range(6):
        ax[i, 0].pcolormesh(Y, Z, obspB[loop, :, :], cmap=cmap, norm=norm)
        ax[i, 1].pcolormesh(Y, Z, impB[loop, :, :], cmap=cmap, norm=norm)
        ax[i, 2].pcolormesh(Y, Z, obspB[loop, :, :] - impB[loop, :, :], cmap=cmap, norm=norm)
        ax[i, 0].text(-5.5, 4.5, f'{angles[loop]:4.1f} deg', color='white', weight='bold', size='large')
        loop += 1
    
    for i in range(6):
        ax[i, 3].pcolormesh(Y, Z, obspB[loop, :, :], cmap=cmap, norm=norm)
        ax[i, 4].pcolormesh(Y, Z, impB[loop, :, :], cmap=cmap, norm=norm)
        ax[i, 5].pcolormesh(Y, Z, obspB[loop, :, :] - impB[loop, :, :], cmap=cmap, norm=norm)
        ax[i, 3].text(-5.5, 4.5, f'{angles[loop]:4.1f} deg', color='white', weight='bold', size='large')
        loop += 1
    ...
    fig.supxlabel('x [R$_\odot$]')
    fig.supylabel('y [R$_\odot$]')

    ax[0, 0].set_title('LASCO')
    ax[0, 3].set_title('LASCO')
    ax[0, 1].set_title('SYN')
    ax[0, 4].set_title('SYN')
    ax[0, 2].set_title('LASCO - SYN')
    ax[0, 5].set_title('LASCO - SYN')
...
if __name__ == '__main__':
    ...
    # LASCO
    filename = 'models/lasco_siren_h256_nh8_w15.0.pth'
    doplot_lasco(filename)
    corona, impB = doplot_lasco_synthesis(filename)
